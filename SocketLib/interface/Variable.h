...
namespace commproto
{
	namespace data
	{
		...
		class VariableMessage : public TypeBase
		{
		public:
			...
			{
				return TypeBase::getSize()
					+ sizeof(valueType)
					+ sizeof(index);
			}
		};

		DEFINE_DATA_TYPE(VariableMessage);
		...
		public:
			...
			uint32_t getSize() const override
			{
				return VariableMessage::getSize() + sizeOf(value);
			}
		};
		...
		public:
			template <typename T>
			Message serialize(TypeBase && data) const
			{
		...
		public:
			...
			VariableParser(const HandlerHandle& handler_)
				: Parser(handler_)
			{
			}

			template <typename T>
			void parse(socketlib::ByteStream && msg, const uint32_t index, const ValueType type)
			{
				T value{};
				msg.read(value);
			...
			void parse(socketlib::ByteStream && msg) override
			{
				uint8_t valueType = 0;
				uint32_t index = 0;
				msg.read(valueType);
